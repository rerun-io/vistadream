...
def prepare_fill_empty_prompt(
    img: Tensor,
    prompt: str | list[str],
    ae: AutoEncoder,
    img_cond: UInt8[np.ndarray, "h w 3"],
    ...
    return_dict = prepare_empty_prompt(img, prompt)
    return_dict["img_cond"] = img_cond.to(img.device)
    return return_dict


def prepare_empty_prompt(img: Tensor, prompt: str | list[str]) -> dict[str, Tensor]:
    bs, c, h, w = img.shape
    if bs == 1 and not isinstance(prompt, str):
        bs = len(prompt)
    ...
    if not os.path.exists(txt_pth):
        txt_pth = "../ckpt/txt.pt"
    txt = torch.load(txt_pth).to(img.device, dtype=img.dtype)
    if txt.shape[0] == 1 and bs > 1:
        txt = repeat(txt, "1 ... -> bs ...", bs=bs)
    txt_ids = torch.zeros(bs, txt.shape[1], 3)
    vec_pth = "./ckpt/vec.pt"
    if not os.path.exists(vec_pth):
        vec_pth = "../ckpt/vec.pt"
    vec = torch.load(vec_pth).to(img.device, dtype=img.dtype)
    if vec.shape[0] == 1 and bs > 1:
        vec = repeat(vec, "1 ... -> bs ...", bs=bs)
