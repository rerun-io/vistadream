...
class UpSample(nn.Module):
    def __init__(
        self, in_channels, out_channels, activate_before="none", activate_after="none", upsample_type="deconv"
    ):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
                    ...
                    if downsample_layer == "downsample":
                        layers.append(
                            DownSample(
                                in_ch_,
                                out_ch_,
                                activate_before="relu",
                                activate_after="none",
                                downsample_type="conv",
                                # partial_conv=partial_conv,  # Removed undefined variable
                            )
                        )
    ...
    def __init__(
        ...
        **ignorekwargs,
        ...
        while stride > 1:
            ...
            layers_ = []
            layers_.append(
                UpSample(in_ch_, out_ch_, activate_before="relu", activate_after="none", upsample_type=upsample_type)
            )
