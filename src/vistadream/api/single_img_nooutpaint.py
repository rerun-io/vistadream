...
class SingleImagePipeline:
    ...
    def _render_splats(self):
        ...
        # render
        for i, cam_T_world in enumerate(tqdm(cam_T_world_traj, desc="Rendering frames"), start=0):
            ...
            # edges_mask: Bool[np.ndarray, "h w"] = depth_edges_mask(dpt, threshold=0.1)
    ...
    def _initialize(self) -> None:
        ...
        input_depth_hw[np.isnan(input_depth_hw) | np.isinf(input_depth_hw)] = 0
        """
        ...
        input_edges_mask: Bool[np.ndarray, "H W"] = depth_edges_mask(input_depth_hw, threshold=0.01)
        # inpaint/outpaint mask without edges (True where inpainting is applied, False near edges and where no inpainting)
        outpaint_wo_edges: Bool[np.ndarray, "H W"] = input_mask & ~input_edges_mask

        input_k33: Float[np.ndarray, "3 3"] = input_rel_depth.K_33.copy()
        ...
        self.scene: Gaussian_Scene = GS_Train_Tool(self.scene, iters=10)(self.scene.frames, log=True)
